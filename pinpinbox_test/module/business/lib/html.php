...
class html {
	...
	function htmlEditorKit(array $attr, $text=null) {
		static $pass;$html = null;$js = null;$id = (isset($attr['id']) && trim($attr['id']) !== '')? trim($attr['id']) : null;
	...
	function image($attr, array $allow_extension=null, $width=null, $height=null) {
		...
		if (!empty($src)) {
			...
			$image_thumbnail_url = fileinfo($Image->set($fileinfo['path'])->setSize()->save())['url'];
		}
	...
	function selectKit(array $attr, array $option, $selected_value=null, $disabled_value=null) {
		...
		if (!$pass) {
			...
			$this->set_css(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/chosen_v1.6.2/chosen.min.css', 'href');
			$this->set_css(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/chosen_v1.6.2/websemantics-Image-Select/ImageSelect.css', 'href');
			$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/chosen_v1.6.2/chosen.jquery.min.js', 'src');
			$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/chosen_v1.6.2/websemantics-Image-Select/ImageSelect.jquery.js', 'src');
		}
	...
	function set_jquery_validation() {
		...
		if (!$pass) {
			...
			$this->set_css(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/jquery-validation-1.15.0/validate.css', 'href');
			$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/jquery-validation-1.15.0/jquery.validate.min.js', 'src');
			$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/jquery-validation-1.15.0/additional-methods.min.js', 'src');
			
			switch (Model('lang')->getSession()) {
				case 'zh_TW':
					$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/jquery-validation-1.15.0/localization/messages_zh_TW.min.js', 'src');
					break;
			}
