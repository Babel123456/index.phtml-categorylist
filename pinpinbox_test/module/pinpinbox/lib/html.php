...
class html {
	...
	function __construct() {
		...
		echo "<script>console.log(".json_encode("\module\pinpinbox\lib\html.php:start(html產生器)".date ("Y-m-d H:i:s" , mktime(date('H'), date('i'), date('s'), date('m'), date('d'), date('Y')))).");</script>";
	...
	function browseKit(array $attr) {
		...
		if ($selector === null) throw new Exception("[".__METHOD__."] Parameters error");
		
		static $pass;$html = null;$js = null;
		if (!$pass) {
			$pass = true;
			$this->set_css(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/PhotoSwipe-master/dist/photoswipe.css', 'href');
			$this->set_css(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/PhotoSwipe-master/dist/default-skin/default-skin.css', 'href');
			$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/PhotoSwipe-master/dist/photoswipe.min.js', 'src');
			$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/PhotoSwipe-master/dist/photoswipe-ui-default.min.js', 'src');
			
			//移出 form 之外, 不然每一個動作皆會觸發 submit 事件
			self::$js .= '
			$(function() {
				$("body").append(
		...
		$(function() {
			$(document).on("click", "'.$selector.'", function(e) {
				...
				var items = [], that0 = $(this), index = 0;
				$("'.$selector.'").each(function(k0, v0) {
					var that1 = $(this),
			        size = that1.data("size").split("x");
	                width = size[0],
	                height = size[1];
						
				    var item = {
				        src: that1.prop("href"),
				        msrc: that1.find("img").prop("src"),
				        w: width,
				        h: height,
				        title: that1.prop("title"),
				    }
	...
	function ckeditor($attr, $text=null) {
		...
		$(window).load(function(){
			CKEDITOR.replace("'.$id.'", {
				filebrowserUploadUrl : "'.\frontstageController::url('upload', 'ckeditor', array('class'=>M_CLASS)).'",
				filebrowserImageUploadUrl : "'.\frontstageController::url('upload', 'ckeditor', array('class'=>M_CLASS)).'",
				filebrowserFlashUploadUrl : "'.\frontstageController::url('upload', 'ckeditor', array('class'=>M_CLASS)).'",
	...
	function datalistKit() {
		static $pass;
		if (!$pass) {
			$pass = true;
			$this->set_jquery();
			$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/Bootstrap-3-Typeahead-master/bootstrap3-typeahead-modify.min.js', 'src');
		}
	}
	...
	function form($attr, $content) {
		...
		if (!$pass) {
			...
			$this->jbox();
			self::$js .= '
			function formerror(content){return new jBox("Modal",{delayOpen:200,title:"<span class=\"helper-valign-middle\"></span><img class=\"valign-middle\" src=\"'.static_file('images/error.png').'\">"}).setContent(content).open();}function formnotice(content){return new jBox("Modal",{delayOpen:200,title:"<span class=\"helper-valign-middle\"></span><img class=\"valign-middle\" src=\"'.static_file('images/info.png').'\">"}).setContent(content).open();}function formsuccess(content){return new jBox("Modal",{delayOpen:200,title:"<span class=\"helper-valign-middle\"></span><img class=\"valign-middle\" src=\"'.static_file('images/success.png').'\">"}).setContent(content).open();}
	...
	function hidden($attr, $text=null) {
		return array('<input type="hidden" '.$attr.'>'.$text, null);
	}
	...
	function image($attr) {
		...
		if (!$pass) {
			...
			var featherEditor = new Aviary.Feather({
				...
				onSave: function(imageID, newURL) {
					...
					$.post("'.frontstageController::url('upload', 'aviary').'", {
		...
		$html .= '<a id="'.$uniqid.'-a" title="" href="'.$image_url.'"><img id="'.$uniqid.'-img" border="0" data-original="'.$image_thumbnail_url.'"></a>';
		$html .= '</div>';
		...
		$(function(){
			...
			$("#'.$uniqid.'-dialog").dialog({
				...
				buttons: {
					ok: function () {
						var obj = this;
						$.post("'.frontstageController::url('upload', 'image_combine', array('class'=>M_CLASS)).'", {
	...
	function img($attr) {
		...
		if (!$pass) {
			...
			list($html_imagebox, $js_imagebox) = $this->browseKit(array('selector'=>'.'.__FUNCTION__.'-a'));
			$html .= $html_imagebox;
			$js .= $js_imagebox;
		}
		...
		if (is_image(PATH_UPLOAD.$src)) {
			...
			$size = $o_image->getWidth().'x'.$o_image->getHeight();
	...
	function jbox() {
		...
		if (!$pass) {
			$pass = true;
            $this->set_css(URL_ROOT.'js/jBox-0.4.8/jBox.css', 'href');
            $this->set_css(URL_ROOT.'js/jBox-0.4.8/plugins/Confirm/jBox.Confirm.css', 'href');
            $this->set_js(URL_ROOT.'js/jBox-0.4.8/jBox.min.js', 'src');
            $this->set_js(URL_ROOT.'js/jBox-0.4.8/plugins/Confirm/jBox.Confirm.min.js', 'src');
		}
	}
	...
	function jquery_cookie() {
		...
		if (!$pass) {
			$pass = true;
			$this->set_jquery();
			$this->set_js(URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/jquery-cookie-master/src/jquery.cookie.js', 'src');
		}
	}
	...
	function number($attr) {
		return array('<input type="number" '.$attr.'>', null);
	}
	...
	function password(array $attr) {
		return array('<input type="password" '.array2htmlattr($attr).'>', null);
	}
	...
	function set_jquery() {
		self::$js_src['jquery'] = URL_STATIC_FILE.M_PACKAGE.'/'.SITE_LANG.'/js/jquery-1.11.3.min.js';
	}
	...
	function set_jquery_validation() {
		...
		if (!$pass) {
			...
			switch ((new \langModel)->getSession()) {
				case 'zh_TW':
	...
	function submit_act($attr, $submit_act) {
		...
		$html .= '<input type="button" '.$attr.' onclick="document.getElementById(\'submit_act\').value = \''.$submit_act.'\';this.disabled = true;this.form.submit();">';
	...
	function upload($attr, array $allow_extension=array()) {
		static $pass;
		$html = null;$js = null;
		...
		if (!$pass) {
			...
			function fileupload_process(obj, files) {
				...
				$.ajax({
					url: "'.frontstageController::url('upload', 'upload', array('class'=>M_CLASS)).'",
					type: "POST",
					xhr: function(){
