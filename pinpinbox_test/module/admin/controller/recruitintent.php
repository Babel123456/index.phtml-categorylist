<?php
class recruitintentController extends backstageController {
	...
	function form() {
		...
		switch ($_GET['act']) {
			...
			case 'edit':
				...
				$m_recruitintent = Model(M_CLASS)->where(array(array(array(array(M_CLASS.'_id', '=', $M_CLASS_id)), 'and')))->fetch();
				
				//form
				$name = $m_recruitintent['name'];
				$reply = $m_recruitintent['reply'];
				$a_feedback = json_decode($m_recruitintent['feedback'], true);
				$act = $m_recruitintent['act'];
				$inserttime = $m_recruitintent['inserttime'];
				$modifytime = $m_recruitintent['modifytime'];
				$modifyadmin_name = adminModel::getOne($m_recruitintent['modifyadmin_id'])['name'];
		...
		$a_act = array();
		foreach (json_decode(Core::settings('RECRUITINTENT_ACT'), true) as $k0 => $v0) {
			$a_act[] = array(
					'name'=>'act',
					'value'=>$k0,
					'text'=>$v0,
	...
	function json() {
		...
		foreach ($fetchAll as &$v0) {
			...
			$v0['recruitX'] = Model('recruit')->column(array('count(1)'))->where(array(array(array(array(M_CLASS.'_id', '=', $v0[M_CLASS.'_id'])), 'and')))->fetchColumn();
		}
