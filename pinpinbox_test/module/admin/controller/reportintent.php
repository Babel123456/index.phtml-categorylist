<?php
class reportintentController extends backstageController {
	...
	function form() {
		...
		switch ($_GET['act']) {
			...
			case 'edit':
				...
				$m_reportintent = Model(M_CLASS)->where(array(array(array(array(M_CLASS.'_id', '=', $M_CLASS_id)), 'and')))->fetch();
				
				//form
				$name = $m_reportintent['name'];
				$reply = $m_reportintent['reply'];
				$a_feedback = json_decode($m_reportintent['feedback'], true);
				$sequence = $m_reportintent['sequence'];
				$act = $m_reportintent['act'];
				$inserttime = $m_reportintent['inserttime'];
				$modifytime = $m_reportintent['modifytime'];
				$modifyadmin_name = adminModel::getOne($m_reportintent['modifyadmin_id'])['name'];
		...
		list($html, $js) = parent::$html->ckeditor('id="reply" name="reply"', $reply);
		$column[] = array('key'=>_('Reply'), 'value'=>$html);
		parent::$html->set_js($js);
		...
		$a_act = array();
		foreach (json_decode(Core::settings('REPORTINTENT_ACT'), true) as $k0 => $v0) {
			$a_act[] = array(
					'name'=>'act',
					'value'=>$k0,
					'text'=>$v0,
	...
	function json() {
		...
		foreach ($fetchAll as &$v0) {
			...
			$v0['reportX'] = Model('report')->column(array('count(1)'))->where(array(array(array(array(M_CLASS.'_id', '=', $v0[M_CLASS.'_id'])), 'and')))->fetchColumn();
		}
