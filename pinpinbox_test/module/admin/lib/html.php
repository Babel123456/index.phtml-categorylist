...
class html
{
    ...
    {
        return ['<a ' . $attr . '>' . $text . '</a>', null];
    }

    function back($attr)
    {
        return ['<input type="button" onclick="history.go(-1);return true;" ' . $attr . '>', null];
    }
    ...
    function browseKit(array $attr)
    {
        ...
        if (!$pass) {
            ...
            $this->set_css(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/lightGallery-master/dist/css/lightgallery.min.css', 'href');
            $this->set_css(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/lightGallery-master/dist/css/lightgallery-custom.min.css', 'href');
            ...
            $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/lightGallery-master/dist/js/lg-audio.min.js', 'src');
            $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/lightGallery-master/dist/js/lg-subhtml.min.js', 'src');
    ...
    function ckeditor($attr, $text = null)
    {
		...
		$(window).load(function(){
			CKEDITOR.replace("' . $id . '", {
				filebrowserUploadUrl : "' . \backstageController::url('upload', 'ckeditor', ['class' => M_CLASS]) . '",
				filebrowserImageUploadUrl : "' . \backstageController::url('upload', 'ckeditor', ['class' => M_CLASS]) . '",
				filebrowserFlashUploadUrl : "' . \backstageController::url('upload', 'ckeditor', ['class' => M_CLASS]) . '",
    ...
    function dynamictable($function, $attr, $array = [])
    {
        static $pass1;
        $return = null;
        $js = null;
    ...
    {
        return ['<input type="email" ' . array2htmlattr($attr) . '>', null];
    }
    ...
    function htmlEditorKit(array $attr, $text = null)
    {
        static $pass;
        $html = null;
        $js = null;
        $id = (isset($attr['id']) && trim($attr['id']) !== '') ? trim($attr['id']) : null;

        if ($id === null) throw new \Exception('Param error.');

        if (!$pass) {
            $pass = true;
            $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/ckeditor_4.5.10_full/ckeditor.js', 'src');
        }

        $html .= '<textarea ' . array2htmlattr($attr) . '>' . $text . '</textarea>';

        /**
         * 2016-07-22 Lion: 增加 allowedContent: true, 讓各標籤的 data-* 屬性得以作用, 再由 on 控制, 制止 img 的 style, 使 css 得以作用; 配合 owl.carousel 套件 lazyLoad 寫入特定 class 和 data-
         */
        $js .= '
			$(function(){
				CKEDITOR.replace("' . $id . '", {
				                ...
				                elements: {
				                    img: function(el){
										if (el.attributes.style) {
											...
											var match = /(?:^|\s)width\s*:\s*(\d+)px/i.exec(el.attributes.style),
												width = match && match[1];
						
											// Get the height from the style.
											match = /(?:^|\s)height\s*:\s*(\d+)px/i.exec(el.attributes.style);
											...
											if (!el.attributes.style.length) delete el.attributes.style;
										}
							
										//配合 owl.carousel 套件 lazyLoad
										if (el.attributes.src) {
											el.addClass("lazyOwl");
											el.attributes["data-src"] = el.attributes.src;
										}
				                    }
				                }
				            });
    ...
    function image($attr, array $allow_extension = null, $width = null, $height = null)
    {
            ...
            self::$js .= '
				var featherEditor = new Aviary.Feather({
					...
					onSave: function(imageID, newURL) {
						...
						$.post("' . \backstageController::url('upload', 'aviary') . '", {
        ...
        if (!empty($src)) {
			...
			$image_thumbnail_url = ($fileinfo['extension'] == 'svg') ? $fileinfo['url'] : fileinfo((new \Core\Image)->set($fileinfo['path'])->setSize()->save())['url'];
        }
		...
		$(function(){
			...
			$("#' . $uniqid . '-dialog").dialog({
				...
				buttons: {
					ok: function () {
						var obj = this;
						$.post("' . \backstageController::url('upload', 'image_combine', ['class' => M_CLASS]) . '", {
    ...
    function img($attr)
    {
        ...
        if (is_image(PATH_UPLOAD . $src)) {
            ...
            $i_image = $Image->set(PATH_UPLOAD . $src);
    ...
    function phoneKit(array $attr)
    {
        ...
        if (!$pass) {
            ...
            $this->set_css(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/intl-tel-input-10.0.2/css/intlTelInput.css', 'href');
            $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/intl-tel-input-10.0.2/js/intlTelInput.min.js', 'src');
        }

        $html .= '<input type="tel" ' . array2htmlattr($attr) . '>';

        $js .= '
		$(function(){
			$("#' . $attr['id'] . '").intlTelInput({
				initialCountry: "auto",
				...
				utilsScript: "' . URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/intl-tel-input-10.0.2/js/utils.js"
			});
		});';
    ...
    function radiotable($width, $height, $col, $array, $checkedvalue = null, $required = false)
    {
        static $pass;
        $html = null;
        $js = null;
        ...
        foreach ($array as $v1) {
            ...
            list($radio_html, $radio_js) = $this->radio('id="' . $uniqid . '_input" ' . $name . ' ' . $value . ' ' . $checked . ' ' . $required, $text);
    ...
    {
        return ['<input type="button" onclick="window.location.reload();return true;" ' . $attr . '>', null];
    }
    ...
    function selectKit(array $attr, array $option, $selected_value = null, $disabled_value = null)
    {
        ...
        if (!$pass) {
            ...
            $this->set_css(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/chosen_v1.8.3/chosen.min.css', 'href');
            $this->set_css(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/chosen_v1.8.3/websemantics-Image-Select/ImageSelect.css', 'href');
            $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/chosen_v1.8.3/chosen.jquery.min.js', 'src');
            $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/chosen_v1.8.3/websemantics-Image-Select/ImageSelect.jquery.js', 'src');
        }
        ...
        foreach ($option as $v0) {
            ...
            $html .= '<option value="' . $v0['value'] . '" ' . array2htmlattr(isset($v0['attribute']) ? $v0['attribute'] : []) . ' ' . $selected . ' ' . $disabled . '>' . $v0['text'] . '</option>';
        }
        $html .= '</select>';
    ...
    function set_jquery_validation()
    {
        ...
        if (!$pass) {
            ...
            $this->set_css(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/jquery-validation-1.17.0/validate.css', 'href');
            $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/jquery-validation-1.17.0/jquery.validate.min.js', 'src');
            $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/jquery-validation-1.17.0/additional-methods.min.js', 'src');

            switch (Model('lang')->getSession()) {
                case 'zh_TW':
                    $this->set_js(URL_STATIC_FILE . M_PACKAGE . '/' . SITE_LANG . '/js/jquery-validation-1.17.0/localization/messages_zh_TW.min.js', 'src');
                    break;
            }
    ...
    function text($attr)
    {
        return ['<input type="text" ' . $attr . '>', null];
    }

    function textarea($attr, $text = null)
    {
        return ['<textarea ' . $attr . '>' . $text . '</textarea>', null];
    }
        ...
        if (!$pass) {
            ...
            self::$js .= '
				function fileupload_process(obj, files) {
					...
					$.ajax({
						url: "' . \backstageController::url('upload', 'upload', ['class' => M_CLASS]) . '",
						type: "POST",
						xhr: function(){
