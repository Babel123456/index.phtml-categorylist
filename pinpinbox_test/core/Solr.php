...
class Solr
{
    ...
    function fetch_logic($fetch_case, $expire)
    {
        ...
        if (!empty(self::$order)) foreach (self::$order as $k0 => $v0) {
        ...
        switch ($fetch_case) {
            case 'fetch':
                //2018-01-11 Lion: 比照 \PDO，fetch 在沒找到東西時是 boolean false
                $data = isset($response->response->docs[0]) ? (array)$response->response->docs[0] : false;
                break;

            case 'fetchAll':
                //2018-01-11 Lion: 比照 \PDO，fetchAll 在沒找到東西時是空陣列
                $data = [];
        ...
        if ($where) {
            foreach ($where as $v0) {
                ...
                foreach ($filters as $v1) {
                    ...
                    //2018-01-10 Lion: Solr datetime format
                    ...
                    switch ($operator) {
                        ...
                        case '>=':
                            $tmp0[] = $field . ':[' . \SolrUtils::escapeQueryChars($value) . ' TO *]';
                            break;

						case 'like' :
							$tmp0[] = $field . ':' . \SolrUtils::escapeQueryChars($value). ' || '. \SolrUtils::escapeQueryChars($value). '*';
							break;
